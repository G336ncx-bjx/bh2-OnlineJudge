...
<h3>2.2 评测进程回收与内存采样（实战踩坑）</h3>
<div class="box warn">
  <strong>问题一：Windows 下评测队列永久卡死。</strong>超时后 <code>finally</code> 块里无界 <code>await proc.wait()</code>，在 Windows Proactor 事件循环下，<code>communicate()</code> 被 <code>wait_for</code> 取消后残留未完成的 wait future，导致后续 <code>proc.wait()</code> 永久挂起，进而卡死整个单 worker 串行评测队列（后续所有提交永远 pending）。改为有界 <code>asyncio.wait_for(proc.wait(), timeout=5.0)</code> + 兜底 <code>_kill</code> 强杀。
</div>
<div class="box warn">
  <strong>问题二：内存峰值恒为 0。</strong>最初在进程结束后才读 RSS，但进程退出后 <code>memory_info().rss</code> 已读不到。改为独立监控协程 <code>_monitor_memory</code> 在运行期间持续采样（含子进程），记录峰值，超限立即杀进程。
</div>

<h3>2.3 路径含空格的处理（实战踩坑）</h3>
<div class="box warn">
...
<ul>
  ...
  <li><code>check_rate_limit(user_id, problem_id)</code>：<strong>单人单题</strong>维度限流，同一用户对同一题目 1 分钟内提交 ≥ 3 次则返回 False（触发 429），不同题目互不影响；管理员免限流。</li>
</ul>
...
<h3>2.6 AI 命题模块（进阶，核心难点）</h3>
<p>AI 模块采用 <strong>OpenAI 兼容 chat completions</strong> 接口（<code>httpx</code> 异步调用），支持任意 provider 可配置切换（实测接入 DeepSeek 与 TokenHub 两个提供商，配置仅需改 provider_url / model / api_key）。命题流程采用 <strong>分阶段生成</strong>策略，每个阶段都写入任务 JSON 的 <code>progress</code> 字段，前端自动轮询拉取最新进度：</p>
...
<div class="box tip">
  <strong>难点一：大模型输出超长被截断。</strong>早期把「整题 + 测试点」一次性让模型生成，输出超过 <code>max_tokens</code> 被截断，导致 JSON 解析失败（实测两次失败：输出 4096、8191 token 均被截断）。解决思路是<strong>分阶段生成</strong>（主体与测试点分开调用），从根源上避免单次输出过长；同时增强 <code>parse_problem_json</code> 的截断兜底修复（顶层逗号截断 / 补闭合括号 / 逐字段回退三种策略）。改造后实测成功出题。
</div>
...
<h3>2.7 访问审计（评测日志可见性）</h3>
<p>每条 <code>GET /api/submissions/{id}/log</code> 请求都记录到 <code>data/audit_logs.json</code>，包含访问者、目标提交、时间、访问状态。访问权限三级：</p>
<ul>
  <li>提交者本人</li>
  <li>管理员</li>
...
<h3>2.8 存储层读缓存（性能优化）</h3>
<div class="box warn">
  <strong>问题：</strong>评测列表接口逐个读文件——193 条提交 ×（<code>get_submission</code> + <code>get_problem</code>）约 386 次文件 I/O + JSON 解析，实测接口耗时 <strong>1.6 秒</strong>，前端点开评测记录/详情要等一两秒。
</div>
<p><strong>解决方案</strong>：给 <code>_read_json</code> 加一层基于文件 <code>mtime + size</code> 的读缓存，文件未变化直接返回缓存；写操作通过 <code>os.replace</code> 原子替换（必然改变 mtime）并主动失效缓存。该方案零风险且对所有读操作透明生效：</p>
<pre><code>def _read_json(path: str, default: Any) -> Any:
    st = os.stat(path)
...
<h3>2.10 统计口径与数据一致性（按验收反馈修正）</h3>
<p>验收前按老师同步的口径核查，发现三处统计/清理语义与预期不符，逐一修正：</p>

<div class="box warn">
  <strong>修正一：通过数「一个题目贡献一次」。</strong>原实现每次满分提交都 <code>resolve_count += 1</code>，同一用户对同一题重复 AC 会重复计数。改为用户数据新增 <code>resolved_problems</code> 集合：题目首次满分时才 +1，重复 AC 不重复计数。同时做了<strong>旧数据惰性重建</strong>——老用户首次评测时自动扫描其历史提交，把旧的重复计数修正为「一题一次」的准确值（实测重建结果与独立扫描完全一致）。rejudge 分数回退不回退计数，保持「曾经 AC 即通过」语义。
</div>

<div class="box warn">
  <strong>修正二：删除题目级联清理。</strong>原实现只删题目 JSON，该题的提交记录与审计日志残留。改为级联删除：题目 JSON + 该题全部提交文件 + <code>audit_logs.json</code> 中该题记录一并清除，并从用户 <code>resolved_problems</code> 集合移除该题（<code>resolve_count</code> 保留历史贡献不递减）。边界处理：评测 worker 在评测前后双重检查题目存在性，<strong>评测进行中删除题目时丢弃残留任务</strong>，避免评测结果在级联删除后重新写回文件（实测无残留）。
</div>
...
<table>
  ...
  <tr><td>MLE 提交</td><td>分配 422MB 内存</td><td>所有测试点 MLE，进程被终止</td><td class="ok">通过</td></tr>
  ...
  <tr><td>参数校验</td><td>缺必填字段</td><td>400（统一参数错误码）</td><td class="ok">通过</td></tr>
  <tr><td>空样例/测试点</td><td>samples/testcases 空列表</td><td>400 拒绝（无法评测的无效题目）</td><td class="ok">通过</td></tr>
  ...
  <tr><td>频率限制</td><td>同题 1 分钟内提交 ≥ 3 次</td><td>429（单人单题，换题不受影响）</td><td class="ok">通过</td></tr>
  <tr><td>重新评测</td><td>admin PUT rejudge</td><td>状态回 pending，旧日志覆盖</td><td class="ok">通过</td></tr>
  <tr><td>停止评测</td><td>PUT cancel（提交者/管理员）</td><td>评测中/排队中的提交可手动取消，标记 error 提示原因</td><td class="ok">通过</td></tr>
...
<ol>
  ...
  <li><strong>本人端到端验收</strong>：每写完一块立即实测接口，发现 bug 就向 AI 复现现象并让其定位修复；AI 命题用真实大模型（DeepSeek / TokenHub）跑通全链路。</li>
...
<table>
  ...
  <tr><td>验收与测试</td><td>本人</td><td>逐项对照要求文档核对功能，用 curl / 真实大模型出题 / 浏览器实测，发现并定位问题。</td></tr>
