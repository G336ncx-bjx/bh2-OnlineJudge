...
- OJ 系统的功能实现，在9月10日（周四）由助教线下验收（验收形式后续通知，参考第一次大作业）。
- 助教会为每位同学创建好仓库。所有源代码需要在9月10日（周四）课前完成，需在网络学堂提交最后一次 git commit 号。
- 实验报告在 9月10日（周四）晚上23:59 在网络学堂截止提交。
...
**Git 提交规范**：要求尽量按照 https://www.conventionalcommits.org/ 规范编写提交信息，不符合规范的提交将酌情扣分。提交信息应包含类型（如 feat、fix、docs 等）和简洁的描述。
