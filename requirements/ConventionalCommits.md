...
- **fix:** 类型为 `fix` 的提交表示在代码库中修复了一个 bug（这和语义化版本中的 PATCH 相对应）。
- **feat:** 类型为 `feat` 的提交表示在代码库中新增了一个功能（这和语义化版本中的 MINOR 相对应）。
- **BREAKING CHANGE:** 在脚注中包含 `BREAKING CHANGE:` 或 `<类型>(范围)` 后面有一个 `!` 的提交，表示引入了破坏性 API 变更（这和语义化版本中的 MAJOR 相对应）。破坏性变更可以是任意类型提交的一部分。
- 除 `fix:` 和 `feat:` 之外，也可以使用其它提交类型，例如 `@commitlint/config-conventional`（基于 Angular 约定）中推荐的 `build:`、`chore:`、`ci:`、`docs:`、`style:`、`refactor:`、`perf:`、`test:`，等等。
...
- 脚注中除了 `BREAKING CHANGE: <description>`，其它条目应该采用类似 https://git-scm.com/docs/git-interpret-trailers 这样的惯例。
...
本文中的关键词 **“必须（MUST）”**、**“禁止（MUST NOT）”**、**“必要（REQUIRED）”**、**“应当（SHALL）”**、**“不应当（SHALL NOT）”**、**“应该（SHOULD）”**、**“不应该（SHOULD NOT）”**、**“推荐（RECOMMENDED）”**、**“可以（MAY）”** 和 **“可选（OPTIONAL）”**，其相关解释参考 https://www.ietf.org/rfc/rfc2119.txt。
...
8. 在正文结束的一个空行之后，可以编写一行或多行脚注。每行脚注都必须包含一个令牌（token），后面紧跟 `:<space>` 或 `<space>#` 作为分隔符，后面再紧跟令牌的值（受 https://git-scm.com/docs/git-interpret-trailers 启发）。
9. 脚注的令牌必须使用 `-` 作为连字符，比如 `Acked-by`（这样有助于区分脚注和多行正文）。有一种例外情况就是 `BREAKING CHANGE`，它可以被认为是一个令牌。
...
12. 在提交说明中，可以使用 `feat` 和 `fix` 之外的类型，比如：`docs: updated ref docs.`。
13. 工具的实现必须不区分大小写地解析构成约定式提交的信息单元，只有 `BREAKING CHANGE` 必须是大写的。
14. `BREAKING-CHANGE` 作为脚注的令牌时必须是 `BREAKING CHANGE` 的同义词。
...
- 当使用了不在规范中的类型时，例如将 `feat` 写成了 `feet`：在最坏的场景下，即便提交没有满足约定式提交的规范，也不会是世界末日。这只意味着这个提交会被基于规范的工具错过而已。
