...
- **说明**：重新评测需覆盖原 `submission_id` 对应的内容
- **权限**：仅管理员
...
```
> 注意：`compile_cmd` 中的 `src` 和 `exe` 需要是路径（如 `test.cpp` 不是路径，但是 `./test.cpp` 或 `/root/test.cpp` 是路径）
...
- **权限**：仅管理员
- **说明**：其中 `status` 作为返回值，记录这次访问状态。不必记录未登录 / submission 不存在 / 参数错误时访问记录。
- **参数**：
  ...
  - 参数意义与 `GET /api/submissions/` 一致
  - 注意：这里的 `action` 只有 `view_logs` 一个操作
- **响应**：
```json
{
  "code": 200,
  "msg": "success",
  "data": [
    {"user_id": "test", "problem_id": "sum_3_numbers", "action": "view_log", "time": "2024-06-01", "status": "403"}
  ]
}
```
